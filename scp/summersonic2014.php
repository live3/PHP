...
*/
	...
	$state = "select * from Schedules where EventCode ='$code'";
	$result = $db->query($state );
	$cc=0;
	while( $row = $result->fetchArray() ) {
...
//		$complete_array[] = $row['band_id'];
		...
		$name = str_replace("'", "\'", sjis_to_utf8($row['Title']) ) ;
//		$image = sjis_to_utf8($row['image']);
//		$desc = mb_substr($row['member'].$row['description1'].$row['description2'].$row['description3'],0,5999);

		$st_name =  sjis_to_utf8($row['Subtitle']);
		$stage_id = 999;
		foreach($db_stages_array as $db_st => $db_st_id){
			if(conv($st_name) === conv($db_st)){
				$stage_id = $db_st_id;
			}
		}
		
		$day = sjis_to_utf8($row['Date']);
		$day_st = substr($day, 5).'/2014';//"08/02/2014";
...
//osaka
'027','128','032','100','101','014','010','037',
'024','040','012','021','044','011','052','125',
'004','022','043','015','034','077','026','023',
'036','042','053','062','075','050','083','092','127','093',
'132','133','124','141',
'001','002','007','009','057','049','058',
'003','006','016','017','072','029','073','059',
'005','038','018','013','019','031','048','045',
'030','008','095','046','094','096','097','028','098','136'
);
