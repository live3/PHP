...
$metrics = 'ga:users,ga:screenviews,ga:sessions'; // 取得する指標(複数指定する場合は「,」でつなげる

// その他指定する場合は連想配列にまとめる
$optParams = array(
        'dimensions'=> 'ga:networkLocation', // ディメンション(複数指定する場合は「,」でつなげる
        'sort' => '-ga:users', // 何でソートするか(降順の場合は先頭に「-」をつける
...
foreach($users_sv_date as $usd){
	...
	$result['network']=$usd[0];
	$result['users']=$usd[1];
	$result['screenviews']=$usd[2];
	$result['sessions']=$usd[3];
