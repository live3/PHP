...
$db = new SQLite3('club_getter.sqlite');

$db->exec('CREATE TABLE IF NOT EXISTS clubs (id INTEGER PRIMARY KEY ,name TEXT,url TEXT, site_url TEXT, mail TEXT,nameF TEXT, link TEXT,image TEXT,member TEXT, description1 Text, description2 TEXT, rate Text,description3 TEXT )');

$state = "select * from clubs";
$result = $db->query($state );
$start_i = 1;


//メールがあるやつだけ抽出 CSV化　No.4
while( $row = $result->fetchArray() ) {
	if(isset($row['nameF'])){
		$res_mail = $row['nameF'];
		$name = $row['name'];
		$desc = str_replace(',', '、',$row['description1']);
		$name = str_replace(',', '、', $name);
		echo $name.','.$res_mail.','.$desc.'<br />';
	}
}
exit;
...
while( $row = $result->fetchArray() ) {
	...
	$lines =preg_split('/\r?\n/', $file);$start_f=0;
	$detail_start_f=0;$detail='';$detail_line_count=0;
...
//		$line = strip_tags($line);
    	...
    	if($detail_start_f == 1){
			...
			$db->exec("Update clubs set description1 ='$detail' where id = $sqlite_id ");
			echo $detail;
			echo '<hr />';
			break;	   
    	}      		
    	if(strpos( $line,'住所')&&strpos( $line,'<dt')){
    		$detail_start_f=1;
    	}		
	}
//	exit;
...
while( $row = $result->fetchArray() ) {
	...
	$mail = str_replace('/', '', $url);
	$mail = str_replace('http:', '', $mail);
	$mail = str_replace('www.', '', $mail);
	echo $mail;
	echo '<br />';	
	
	$url ='https://www.google.co.jp/search?q=メール+@'.$mail;
	$opts = array(
	'http'=>array(
		'method'=>"GET",
		'user_agent'  => "Mozilla/5.0 (iPhone; U; CPU like Mac OS X; en) AppleWebKit/420+ (KHTML, like Gecko) Version/3.0 Mobile/1C28 Safari/419.3", 
...
//			if(preg_match("%[A-Z0-9+_.-]+@[A-Z0-9.-]+%isu",$line, $match)){
			if(preg_match("%[A-Z0-9+_.-]+@%isu",$line, $match)){
				$res = $match[0].$mail;
				echo $res;
				echo '<hr />';
				$db->exec("Update clubs set nameF ='$res' where id = $sqlite_id ");
				$c_res++;				
				break;
			}			
		}
...
/*
		if(strpos($line,'<h3') && strpos($line,'title="') ){
...
//東京のクラブのURLをげっと　No.1
$host = 'http://www.tokyo-club.net/';
$areas = array('shibuya','ebisu','roppongi','azabu','aoyama','shinjyuku','ikebukuro','ginza','shinkiba','other');
	...
	for($i=1; $i <= 6; $i++ ){
	    ...
	    foreach($lines as $line){
	    	...
	    	if(strpos($line,'<h3') && strpos($line,'title="') ){
				...
				$file_detail = file_get_contents($link, false, $context);
				$lines_detail =preg_split('/\r?\n/', $file_detail);	
				$detail_start_f=0;$detail_link='';
				foreach($lines_detail as $ln){											
			    	if(strpos( $ln,'id="intro')){
			    		$detail_start_f=1;
			    	}
			    	if($detail_start_f == 0){
				    	continue;
					...
					if(strpos($ln,'dd') && strpos($ln,'rel="nofollow') ){
				    	if(preg_match('%href="([^"]*)%',$ln, $match)){
				    		$detail_link = $match[1];
				    	}	
						$db->exec("INSERT INTO clubs (name,url,site_url) VALUES (\"$shop\",\"$detail_link\",\"$link\")");		    					    	
					}			    	
